...
function authenticate(request, response, next) {
	...
	var signed_request = null;//getSignedRequest(request);
...
function parseSignedRequest(payload_string) {
	...
	var sig = payload[0];
	console.log('Signed request response: ' + new Buffer(payload[1], 'base64').toString());
	...
	https.get(opts, function (res) {
		...
		res.on('end', function () {
			if (res.code == 200) {
				...
				var error_obj = JSON.parse(token).error || { message: 'Can\'t get access token', type: 'unknown'};
